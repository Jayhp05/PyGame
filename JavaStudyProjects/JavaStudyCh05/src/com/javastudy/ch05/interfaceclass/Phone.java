...
public interface Phone {
	 ...
	 * 인터페이스를 구현하는 여러 하위 클래스에서 공통으로 사용할 같은 기능의 메서드가
	 * 필요할 때가 있는데 Java7까지는 기능이 같다고 해서 인터페이스에서 완성 메서드를
	 * 정의할 수 없었기 때문에 인터페이스에서 추상 메서드를 정의하고 이 인터페이스를
	 * 구현하는 클래스에서 같은 기능의 메서드를 각각 구현해야 했다. 하지만 Java8부터는     
	 * 아래와 같이 default 메서드를 인터페이스에 정의해서 하위 클래스에서 사용할 있게
	 * 되었으며 default 메서드 또한 하위 클래스에서 오버라이드 해서 사용할 수도 있다.
	 * 그리고 인스턴스 생성없이 사용할 수 있는 static 메서드도 Java8부터 인터페이스에
	 * 정의할 수 있게 되어 인터페이스의 활용성을 더욱 높일 수 있게 되었다.
